...
(async () => {
  ...
  try {
    ...
    await page.locator('.text-box[title="TESTO DIRETTO 6"]').waitFor();
    console.log('QA: direct inline text committed without Add Text or Apply');
    await page.locator('#add-text-mode').click();
    await page.locator('#add-text-mode.is-active').waitFor();
    const directTextBox = page.locator('.text-box[title="TESTO DIRETTO 6"]');
    ...
    await page.locator('#add-text-mode.is-active').waitFor();
    await page.locator('.text-box[title="TESTO DIRETTO 6"]').click();
    await page.locator('.inline-text-editor .inline-text-content').fill('TESTO RIPRESO 6');
    await page.locator('#apply-edit').click();
    await page.locator('.text-box[title="TESTO RIPRESO 6"]').waitFor();
    await page.locator('#add-text-mode.is-active').waitFor();
